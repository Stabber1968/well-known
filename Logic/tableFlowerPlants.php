...
$sIndexTable = "index_flower"; // table 2
...
if ($_GET["lot"]) {
    $sJoin .= " AND ". $table .".lot_ID='" . $_GET["lot"] . "'";
}
...
$columns = array(
    ...
    array('db' => 'number_of_plants', 'dt' => 'td_number_plants',),
    array('db' => 'genetic_ID', 'dt' => 'td_genetic_id',),
    array(
        'db' => 'genetic_ID', 'dt' => 'td_genetic',
        'formatter' => function ($d, $row) {
            $p_general = new General();
    ...
    // array('db' => 'mother_text', 'dt' => 'td_mother_id_text',),
    // array(
    //     'db' => 'mother_id', 'dt' => 'td_mother_id_exist',
    //     'formatter' => function ($d, $row) {
    //         $p_general = new General();
    //         $exist = $p_general->getValueOfAnyTable('plants', 'id', '=', $d);
    //         $exist = $exist->results();
    //         return  sizeof($exist);
    //     }
    // ),
    ...
    array(
        ...
        'formatter' => function ($d, $row) {
            $p_general = new General();
            $roomInfo = $p_general->getValueOfAnyTable('room_flower', 'id', '=', $d);
