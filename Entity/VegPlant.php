...
class VegPlant {
    ...
    public function addPlantToIndexFlower($flowerRoomID,$PlantID,$lot_ID, $genetic_ID) {
        if(!$this->_db->insert('index_flower', array(
            ...
            throw new Exception('Problem addPlantToIndexFlower......');
        }
    }

    public function CreateRelationVegRoomAndPlant($flowerRoomID,$createdPlantQRCode) {

        $createdPlantID = $this->getValueOfAnyTable('plants','qr_code','=',$createdPlantQRCode);
        $createdPlantID = $createdPlantID->results();

        $this->deleteValueOfAnyTable('index_veg','plant_id','=',$createdPlantID[0]->id);
    ...
    public function getPlantsListFromVegRoomID($vegRoomID) {

        $vegPlantsIDList = $this->getValueOfAnyTable('index_veg','room_id','=',$vegRoomID,'plant_id');
        $vegPlantsIDList = $vegPlantsIDList->results();

        return $vegPlantsIDList;
