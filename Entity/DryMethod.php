...
class DryMethod {
    ...
    public function create($fields = array()) {
        ...
        if(!$this->_db->insert('dry_method', $fields)) {
    ...
    public function update($fields = array(), $id = null) {
        if(!$this->_db->update('dry_method', $id, $fields)) {
    ...
    public function find($id = null) {
        if($id) {
            ...
            $data = $this->_db->get('dry_method', array('id', '=', $id));
    ...
    public function getAllOfInfo(){
        ...
        $data = $this->_db->get('dry_method', array('1', '=', '1'));
        if($data->count()) {
            $this->_data = $data->first();
            $this->_count = $data->count();
        }
    ...
    public function delete($id){
        return $this->_db->delete('dry_method', array('id', '=', $id));
    }
    ...
    public function isSame($name){
        if($name) {
            $data = $this->_db->get('dry_method', array('name', '=', $name));
