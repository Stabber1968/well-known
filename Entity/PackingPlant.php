...
class PackingPlant {
    ...
    public function create($fields = array()) {
        if(!$this->_db->insert('vault', $fields)) {
            throw new Exception('There was a problem creating this menu.');
        }
    }

    public function createVault($fields = array()) {
        if(!$this->_db->insert('vault', $fields)) {
            throw new Exception('There was a problem creating this menu.');
        }
    }
    ...
    public function CreateRelationPackingRoomAndPlant($flowerRoomID,$createdPlantQRCode,$prevLot_ID) {
        $createdPlantID = $this->getValueOfAnyTable('plants','qr_code','=',$createdPlantQRCode);
        $createdPlantID = $createdPlantID->results();
        $this->deleteValueOfAnyTable('index_packing','plant_id','=',$createdPlantID[0]->id);
    ...
    public function getPlantsListFromPackingRoomID($packingRoomID) {
        $packingPlantsIDList = $this->getValueOfAnyTable('index_packing','room_id','=',$packingRoomID,'plant_id');
        $packingPlantsIDList = $packingPlantsIDList->results();
        return $packingPlantsIDList;
    }
    ...
    public function UpdateLocationOfIndexTable($location,$lot_ID) {
        $Info = $this->getValueOfAnyTable('index_packing','lot_id','=',$lot_ID);
        $Info = $Info->results();
        if(!$this->_db->update('index_packing', $Info[0]->id, array(
            'room_id' => $location,
        ))) {
            throw new Exception('There was a problem updating.');
        }
    ...
    public function find($id = null) {
        if($id) {
            ...
            $data = $this->_db->get('room_packing', array('id', '=', $id));
    ...
    public function getAllOfInfo(){
        ...
        $data = $this->_db->get('room_packing', array('1', '=', '1'));
        if($data->count()) {
            $this->_data = $data->first();
            $this->_count = $data->count();
        }
    ...
    public function delete($id){
        return $this->_db->delete('room_packing', array('id', '=', $id));
    }
