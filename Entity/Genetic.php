...
class Genetic {
    ...
    public function create($fields = array()) {
        ...
        if(!$this->_db->insert('genetic', $fields)) {
    ...
    public function update($fields = array(), $id = null) {
        if(!$this->_db->update('genetic', $id, $fields)) {
            throw new Exception('There was a problem updating.');
        }
    }


    public function updatePlantNameAtPlantsTable($fields = array(), $id = null) {
        if(!$this->_db->updateNameAtPlants('plants', $id, $fields)) {
            throw new Exception('There was a problem updating.');
        }
    }
    ...
    public function getAllOfInfo(){
        ...
        $data = $this->_db->get('genetic', array('1', '=', '1'));
        if($data->count()) {
            $this->_data = $data->first();
            $this->_count = $data->count();
        }
     ...
     * @return bool
     */
    public function isSameGenetic($genetic_name) {
        if($genetic_name) {
            $data = $this->_db->get('genetic', array('genetic_name', '=', $genetic_name));
